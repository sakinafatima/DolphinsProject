...
def plot_confusion_matrix(cm,
    ...
    plt.xlabel('Predicted label\naccuracy={:1f}; misclass={:1f}'.format(accuracy, misclass))
    plt.show()

train = pd.read_csv("TrainingData/Trainbinary.csv")
print("Shape of Train",train.shape)
train.head()
test = pd.read_csv("TrainingData/Validbinary.csv")
...
#dividingt dataset to features and labels
...
model_rf.fit(Xtrain_smote, Xtrain_smote.values.ravel())
predictions = model_rf.predict(X_test)
from sklearn.metrics import classification_report, confusion_matrix, balanced_accuracy_score
print(confusion_matrix(Y_test,predictions))
print(classification_report(Y_test,predictions))
...
t0 = pl.time.time()
rf = RandomForestClassifier(class_weight='balanced', random_state=49,bootstrap=True, max_depth=80, max_features='sqrt', min_samples_leaf= 3, min_samples_split=8, n_estimators=1000)
# model_rf=GridSearchCV(rf,parameters_RF,cv=3)
...
parameters_SVM={'C': [1, 10, 100, 1000], 'kernel': ['linear','rbf'],'gamma': [0.001, 0.0001]}
t0 = pl.time.time()

sample=SMOTETomek(random_state=49)
Xtrain_sample, Ytrain_sample= sample.fit_sample(X_train, Y_train)
...
GB_model.fit(Xtrain_sample, Ytrain_sample.values.ravel())
